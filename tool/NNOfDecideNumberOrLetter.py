...
def handleData(data):
    for i in range(len(data)):
        ...
        label = data[i][1]
        newLabel = [0] * 2
        if (ord(label) >= ord('0')) and (ord(label) <= ord('9')):
            newLabel[0] = 1
...
if __name__ == "__main__":
    ...
    net = Network.FullConnectedNeuralNetworkUseEntropy([192, 100, 10, 2])
    print('training begin!')
    net.train(trainData, testData, 30, 20, 0.1)
    print('training end!')
    net.save('..\\model\\NNOfDecideNumberOrLetter\\' + time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime()))
